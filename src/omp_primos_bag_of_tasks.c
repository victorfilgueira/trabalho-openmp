...
int main(int argc, char *argv[])
{
    ...
    //
    // Para implementar o bag of tasks, basta utilizar o controle fornecido
    // pela diretiva schedule, que permite dividir as threads em blocos
    // Como a computação do primo vai ficando mais lenta pro final, vamos
    // deixar com a governança dynamic que muda de acordo com o runtime
    omp_set_num_threads(num_threads);
    #pragma omp parallel for schedule(dynamic, NUM_BAG) reduction(+ : count)
    for (i = 3; i <= n; i += 2)
    {
        if (primo(i) == 1)
            count++;
